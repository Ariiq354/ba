...
- **Repository**: Pure DB queries with `Effect.fn("Repo.method")((...) => Effect.tryPromise({ try: async () => ..., catch: (error) => ... }))`. Paging returns `{ total, data }`. Catch DB unique violations via `isUniqueViolation(error)` (`~~/server/utils/pgcode`) to return typed domain errors. Encapsulate `db.transaction` inside repo for single-repo operations (no `tx` param). Parameter `tx` (`tx: DbTransaction | typeof db = db`) is ONLY present on methods participating in multi-step transactions orchestrated by service.
