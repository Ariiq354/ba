...
1. **Repository Layer (`repo.ts`)**: Pure DB queries with `Effect.fn("Repo.method")((...) => Effect.tryPromise({ try: async () => ..., catch: (error) => ... }))`. Single-record lookups use `db.query.<model>.findFirst(...)` (or `tx.query.<model>.findFirst(...)`). Standard `create` operations return `void`. Pagination returns `{ total, data }`. Use `.returning()` on update and delete. Catch DB unique violations via `isUniqueViolation(error)` (`~~/server/utils/pgcode`) to return typed domain errors. Parameter `tx = db` is ONLY present on methods participating in transactions.
2. **Service Layer (`service.ts`)**: Encapsulates business logic, validation, and multi-step transactions using `Effect.fn("Service.method")(function* (...) { ... })` and `yield*`. Yields typed domain errors (`TaggedError`) or generic errors (`ItemNotFoundError`, `ItemsNotFoundError`).
