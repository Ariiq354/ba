...
1. **Repository Layer (`repo.ts`)**: Pure DB queries with `Effect.fn("Repo.method")((...) => Effect.tryPromise({ try: async () => ..., catch: (error) => ... }))`. Single-record lookups use `db.query.<model>.findFirst(...)`. Standard `create` operations return `void`. Pagination returns `{ total, data }`. Use `.returning()` on update and delete. Catch DB unique violations via `isUniqueViolation(error)` (`~~/server/utils/pgcode`) to return typed domain errors. For single-repo mutations across multiple tables, encapsulate `db.transaction` inside repo (no `tx` parameter). Parameter `tx` (`tx = db`, inferred without explicit type annotations) is ONLY present on methods participating in multi-step transactions orchestrated by service.
2. **Service Layer (`service.ts`)**: Encapsulates business logic, validation, and multi-step transactions using `Effect.fn("Service.method")(function* (...) { ... })` and `yield*`. Invokes single-method transactions directly with `yield* Repo.method(...)` and manages `db.transaction(async (tx) => ...)` only when coordinating multiple repo methods or DB utilities. Yields typed domain errors (`TaggedError`) or generic errors (`ItemNotFoundError`, `ItemsNotFoundError`).
3. **API Layer (`server/api/v1/*`)**: Thin route handlers that validate input (`readValidatedBodySafe`, `getValidatedQuerySafe`, `getValidatedRouterParamsSafe`), check guards (`authGuard`, `adminGuard`), and pipe service results to standard 3-property `createError` via `.pipe(Effect.catchTags({...}), Effect.runPromise)`.
4. **Server Utilities (`server/utils/*`)**: Pure/plain TypeScript or async functions returning standard Promises or values without direct Effect-TS runtime dependencies. Any Effect-TS wrapping (`Effect.tryPromise`) is performed strictly within `server/modules/*` (Repo or Service layers).
