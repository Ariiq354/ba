...
1. **Repository Layer (`repo.ts`)**: Pure DB queries with `Effect.fn("Repo.method")((...) => Effect.tryPromise({ try: async () => ..., catch: (error) => ... }))`. Pagination returns `{ total, data }`. Catch DB unique violations via `isUniqueViolation(error)` (`~~/server/utils/pgcode`) to return typed domain errors. Parameter `tx = db` is ONLY present on methods participating in transactions.
