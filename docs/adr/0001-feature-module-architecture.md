...
### 4. Direct Use of `DataTable` Component

- Feature containers (`<FeatureName>Container.vue`) MUST use `~/components/table/DataTable.vue` directly for rendering data tables.
- **Prohibited**: Creating single-use table wrapper components (e.g. `SimpananMutasiTable.vue`, `ApprovalMutasiTable.vue`). Instead, pass `:columns="columns"` from `model.ts` and declare cell template slots directly inside the container component.
...
- Prevention of duplicate logic, single-use wrapper bloat, stale types, and fragmented validation rules.
